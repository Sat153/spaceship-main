...
export async function GET() {
  ...
  const EMAIL = 'mohit@anyasegen.com'
  const PASSWORD = 'MohitAnya2024'
  ...
  try {
    ...
    if (profile) {
      ...
      await admin.from('profiles').insert({ id: data.user.id, email: EMAIL, first_name: 'Mohit', last_name: '', role: 'user', department_id: dept?.id ?? null })
    }
